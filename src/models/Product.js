...
const productSchema = new mongoose.Schema({
  ...
  category: {
    ...
    trim: true,
    enum: {
      values: ["Eletrônicos", "Utilitários", "Acessórios para celular", "Acessórios para carros", "Outros"],
  ...
  images: {
    type: [String], // Define `images` como um array de strings
    validate: {
      validator: function (v) {
        // Verifica se todas as strings no array são URLs válidas
        return v.every(str => /^(https?:\/\/)?([\da-z.+-]+\.[a-z.]{2,6})([/\w.-]*)*\/?$/.test(str));
      },
      message: (props) => `A URL '${props.value}' não é válida!`,
    },
  },
